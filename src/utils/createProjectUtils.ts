...
import { assertDirectoryExists } from '../utils/fileUtils.js';
import { copyDirectory } from "./fileUtils.js";
import { renderTemplate } from "./templateUtils.js";
import { fileURLToPath } from 'url';
import { spawn } from "child_process";

// ES Module support for __dirname
const _dirname = path.dirname(fileURLToPath(import.meta.url));
const templatesDir = path.join(_dirname, '../assets/templates');
assertDirectoryExists(templatesDir);
 ...
 */
export function createProjectDirectory(projectDir: string) {
 ...
 * @param projectDir - Absolute path to the project directory.
 */
export function initializeGitRepository(projectDir: string) {
 ...
 */
export function renderProjectTemplates(projectDir: string, projectName: string) {
 ...
 * @param projectDir - Path to the project directory.
 */
export function copyExamplesDirectory(projectDir: string) {
    const examplesSrc = path.join(_dirname, '../src/assets/examples');
    const examplesDest = path.join(projectDir, 'examples');
    if (fs.existsSync(examplesSrc)) {
        copyDirectory(examplesSrc, examplesDest);
        // fs.copyFileSync(path.join("azure-builder-helper/src/examples", "README/Azure_Builder_Guide.md"), path.join(examplesDest, "Azure_Builder_Guide.md"));
 ...
 */
export function createNpmRcFile(projectDir: string) {
 ...
 * @param projectDir - Absolute path to the project directory.
 */
export function generateAzurePipelineYaml(projectDir: string, projectName: string) {
 ...
 */
export function createDefaultPackageJson(projectDir: string, projectName: string) {
 ...
 */
export function installNpmDependencies(projectDir: string, projectName: string) {
...
/**
 * Creates a README file in the project directory if it doesn't already exist.
 * 
 * @param projectDir - Absolute path to the project directory.
 * @param projectName - Name of the project (used in README content).
 */
export function createProjectReadme(projectDir: string, projectName: string) {
    const readmePath = path.join(projectDir, 'README.md');
    if (!fs.existsSync(readmePath)) {
        const defaultReadme = `# ${projectName}\n\nPulumi project created with Azure Builder Helper`;
        fs.writeFileSync(readmePath, defaultReadme, 'utf8');
    }
}

/**
 * Initializes a Pulumi stack with the specified name.
 * 
 * @param stackName - Name of the stack to initialize.
 */
export function initStack(stackName: string) {
    const stackInit = spawn('pulumi', ['stack', 'init', `vizientinc/${stackName}`])

    stackInit.on('close', (code) => {
        if (code !== 0)
            console.error(chalk.bold.red(`Error initializing stack: StackName: ${stackName}`))
    })
}

/**
 * Initializes multiple Pulumi stacks with the specified names.
 * 
 * @param stackNames - Array of stack names to initialize.
 */
export function initStacks(stackNames: string[]) {
