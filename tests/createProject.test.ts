...
//   const templatesDir = path.join(__dirname, '../src/templates/pulumi');
...
//     const exitSpy = jest.spyOn(process, 'exit').mockImplementation((code?: number) => { throw new Error(`Process exited with code ${code}`); });
